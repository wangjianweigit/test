...
public class ProductControl {
    ...
    */
	...
	/**
    * 【童库-私有商品共享】
    * @api {post} /{project_name}/product/shared_product_list 共享商品库商品列表
    * @apiGroup product
    * @apiDescription  共享商品库商品列表
    * @apiVersion 0.0.1
	...
	* @apiSuccess {number} obj.DISTRICT_TEMPLET_ID 区域模板ID
    */
	@RequestMapping(value = "/shared_product_list", method = RequestMethod.POST)
	@ResponseBody
	public Packet querySharedProductList(HttpServletRequest request) {
		return Transform.GetResult(productService.querySharedProductList(request));
	}
	
	/**
    *【童库-私有商品共享】
    * @api {post} /{project_name}/product/shared_product_edit 添加移除分销商品
    * @apiGroup product
    * @apiDescription  分销商品列表
    * @apiVersion 0.0.1
    * @apiParam {String[]} ids				分销商品ids
    ...
    * @apiSuccess {string} message 接口返回信息说明
    */
	@RequestMapping(value = "/shared_product_edit", method = RequestMethod.POST)
	@ResponseBody
	public Packet sharedProductEdit(HttpServletRequest request) {
		ProcessResult pr = new ProcessResult();
		try{
			pr = productService.sharedProductEdit(request);
		}catch(Exception e){
			pr.setMessage(e.getMessage());
			pr.setState(false);
		}
