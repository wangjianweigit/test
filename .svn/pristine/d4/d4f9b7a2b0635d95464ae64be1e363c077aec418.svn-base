...
public class ProductTypeService {
    ...
    @Value("${sys_dictionary_sync_product_type}")
    private String sys_dictionary_sync_product_type;
	...
	public ProcessResult addProductType(HttpServletRequest request) {
        ...
        try {
            ...
            int sortId = productTypeDao.getMaxSortIdByParentId(Long.parseLong(map.get("parent_id").toString()));
            ...
            syncMap.put("operationType", "editOrAdd");
            syncMap.put("list", list);
            ProcessResult oAPr = HttpClientUtil.postOaSync(oa_service_url + sys_dictionary_sync_product_type, syncMap);
            if (! oAPr.getState()) {
                throw new RuntimeException("同步商品分类数据至OA失败");
            }
        } catch (Exception e) {
            pr.setState(false);
            pr.setMessage(e.getMessage());
    ...
    public ProcessResult batchEditProductType(HttpServletRequest request) {
        ProcessResult pr = new ProcessResult();
        int sortId = 0,parentId = 0;
        Map<String, Object> sortLevelMap = new HashMap<String, Object>();
            ...
            if(list!=null&&list.size()>0){
                for(Map<String, Object> map:list){//重名判断
                    ...
                    if(StringUtils.isEmpty(map.get("id"))){
                        parentId = Integer.parseInt(map.get("parent_id").toString());
                        if(sortLevelMap.containsKey(parentId)){
                            sortId = Integer.parseInt(sortLevelMap.get(parentId).toString());
                            sortLevelMap.put(String.valueOf(parentId), ++sortId);
                        }else{
                            // 获取同级分类最大排序值
                            sortId = productTypeDao.getMaxSortIdByParentId(parentId);
                            sortLevelMap.put(String.valueOf(parentId), ++sortId);
                        }
                        insertList.add(map);
                    }else{
                    ...
                    if(map.get("id") instanceof String && !StringUtils.isEmpty(map.get("id"))){
	...
	public ProcessResult removeProductType(HttpServletRequest request) {
        ...
        try {
            ...
            Map<String,Object> retMap = productTypeDao.queryChildrenByParentId(params);
                ...
                // 同步删除数据至OA
                Map<String, Object> syncMap = new HashMap<String, Object>();
                syncMap.put("operationType", "del");
                syncMap.put("id", params.get("id"));
                ProcessResult oAPr = HttpClientUtil.postOaSync(oa_service_url + sys_dictionary_sync_product_type, syncMap);
                if (! oAPr.getState()) {
                    throw new RuntimeException("同步商品分类数据至OA失败");
                }
