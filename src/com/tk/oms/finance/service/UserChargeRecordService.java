				...
				if(insert_record_num>0){
					...
					if(gr.getState()){
						if (this.userChargeRecordDao.update(params)>0) {
		                    ...
		                    messageMap.put("user_name", recordObj.get("COLLECT_USER_NAME"));
		                    messageMap.put("order_number", recordObj.get("RECORD_NUMBER"));
		                    messageMap.put("balance_money", "0");
		                    messageMap.put("credit_money", "0");
		                    messageMap.put("third_money", recordObj.get("MONEY"));
		                    messageMap.put("third_type", "现金");
		                    messageMap.put("third_number", "");
		                    messageMap.put("pay_seq", 1);
		                    messageMap.put("operate_type", 2);
		                    messageMap.put("pay_type", "tk.cashpay.trade.pay");
