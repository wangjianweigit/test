...
import com.tk.oms.analysis.dao.MemberAnalysisDao;
...
private Log logger = LogFactory.getLog(this.getClass());
	...
	@Resource
	private MemberAnalysisDao memberAnalysisDao;
	@Value("${jdbc_user}")
	 ...
	 * @return
	 */
	public GridResult queryMemberAnalysisList(HttpServletRequest request) {
		...
		try {
			...
			int count=memberAnalysisDao.queryMemberAnalysisCount(params);
			list = memberAnalysisDao.queryMemberAnalysisList(params);
			pr.setState(true);
			pr.setMessage("获取会员分析列表数据成功");
			pr.setObj(list);
			pr.setTotal(count);
		} catch (Exception e) {
			pr.setState(false);
	 ...
	 * @return
	 */
	public ProcessResult queryMemberChartList(HttpServletRequest request) {
        ...
        try {
            ...
            // 查询用户日统计
            List<Map<String, Object>> list = memberAnalysisDao.queryMemberChartList(paramMap);
