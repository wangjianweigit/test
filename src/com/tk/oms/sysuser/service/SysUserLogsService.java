...
public class SysUserLogsService {
	...
	public ProcessResult addSysUserLogs(HttpServletRequest request){
        ...
        try {
			...
			boolean flag = false;
			String post_param = sysUserLogs.getPost_param();
            int len = 1300;
			if(post_param.length() > len) {
                flag = true;
                sysUserLogs.setPost_param(post_param.substring(0, len));
            }
			/**********************插入日志信息****begin**************************/
			...
			if(flag) {
                ...
                detailMap.put("post_param_extra",post_param);
